      ...
      <div className="grid grid-cols-7 gap-2">
        ...
        {dayList.map((k) => {
          ...
          return (
            <button
              key={k}
              className={`card p-2 min-h-[100px] text-left relative ${profit ? "ring-1 ring-emerald-400 bg-emerald-50" : ""} ${loss ? "ring-1 ring-red-400 bg-red-50" : ""} ${disabled ? "opacity-60" : "hover:shadow"}`}
            ...
            >
              ...
              {hasPnl && (
                <>
                  <div className={`text-xs font-medium mt-1 ${profit ? "text-emerald-700" : loss ? "text-red-700" : "text-slate-600"}`}>
                    P&L: {p.realized}
                  </div>
                  {unrealizedNum !== 0 && (
