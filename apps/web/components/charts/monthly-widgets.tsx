        ...
        <div className="h-64">
            <ResponsiveContainer>
                <BarChart data={monthly}>
                    ...
                    <YAxis
                        tickFormatter={(value) =>
                            new Intl.NumberFormat(undefined, { notation: "compact", compactDisplay: "short" }).format(value)
                        }
                    />
...
export function NavByMonthChart({ monthly }: { monthly: { month: string; navEnd: number | null }[] }) {
    ...
    const showDots = dataWithCarryForward.length <= 2;
            ...
            {hasData ? (
                <ResponsiveContainer>
                    <LineChart data={dataWithCarryForward}>
                        ...
                        />
                        <Tooltip content={<CustomTooltip />} />
                        <Line type="monotone" dataKey="navEnd" stroke="#10b981" strokeWidth={2} dot={showDots} animationDuration={3000} animationEasing="ease-out" />
                    </LineChart>
                </ResponsiveContainer>
            ) : (
